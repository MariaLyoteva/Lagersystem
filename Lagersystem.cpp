...
void Lagersystem::addCustomer(const std::shared_ptr<Kunde> &customer) {
...
void Lagersystem::addProdukt(const std::shared_ptr<Produkt> &produkt) {
...
void Lagersystem::addLagerbestand(int productId, const Lager &stockLevel) {
    ...
    if (produktIter == produktkatalog.end()) {
        throw std::runtime_error(
                "Error: Produkt with ID " + std::to_string(productId) + " not found in produktkatalog.");
    }
...
void Lagersystem::printLagerbestand() const {
    for (const auto &pair: lagerbestand) {
        ...
        const Produkt *produkt = produktkatalog.at(produktId).get();
...
std::string Lagersystem::getAllAsString() const {
    ...
    for (const auto &pair: produktkatalog) {
        Produkt *produkt = pair.second.get();
